"""History indexer for FindTab v2 - LLM-enriched bookmark pipeline."""
...
from .enricher_v2 import BookmarkEnricher
...
class BookmarkIndexer:
    ...
    def __init__(
        ...
        Args:
            ...
            ollama: Ollama client for classification/enrichment
        """
        self.db = db
    ...
    def run_incremental_index(self, force_full: bool = False) -> IndexingStats:
        ...
        if force_full:
            window_start = datetime.now() - __import__('datetime').timedelta(
                days=self.settings.findtab_bootstrap_days
            )
        else:
        ...
        hours_back = int((window_end - window_start).total_seconds() / 3600) + 1
        
        # Stage 1: Extract from browsers
        all_entries = self._extract_history(hours_back)
